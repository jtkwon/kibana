...
export const asyncRecurseFetch = (log: ToolingLog) => (supertest: SuperTest<any>) => async (
  names: any,
  i: number = 1
) => {
  const appName = names[i - 1];
  await fetchSavedObjects()(appName)(log)(supertest);

  recurseEither(names.length)(i) // Recurses only when names.length is less than i
    .fold(id, async () => {
      i++;
      await asyncRecurseFetch(log)(supertest)(names, i);
    });
};
